...
const int qxx = 20, qzz = 20, showxx = 2, showzz = 3; int bests[showxx * showzz][2];
float TimeToStep = 50;  int thebest[2], theworst[2];
...
struct Tree
{
            ...
            for (int x = 0; x < TreeX; x++) for (int y = 0; y < TreeY; y++) for (int z = 0; z < TreeZ; z++)
            {
                ...
                if (themap.themap[x][y][z] == -1)
                {
                    ...
                    else
                    {
                        ...
                        energy += pow(7.2, (1.0 + double(TreeY / 2 - y) * 0.06)) / ((double)need * 0.012 + 1.0f);
                    }
                }
            }
            themap = themapbuf;
    ...
    void Evolve()
    {
        for (int i = 0; i < 8; i++) if (rand() % 8 == 0) Genom[rand() % GenLength][rand() % 6] = rand() % (GenLength * 3);
    }
};
...
bool IsOkayUpY(int x, int z, int xx, int yy, int zz)
{
    return ((yy < (TreeY / 2) && trees[x][z].themap.themap[xx][yy + 1 + (TreeY / 2)][zz] == -2) || yy == (TreeY / 2)) &&
        ((yy > 1 && trees[x][z].themap.themap[xx][yy - 1 + (TreeY / 2)][zz] == -2) || yy == 1);
}
bool IsOkayUpX(int x, int z, int xx, int yy, int zz)
{
    return IsOkayUpY(x, z, xx, yy, zz) &&
        (((xx < TreeX - 1 && trees[x][z].themap.themap[xx + 1][yy + (TreeY / 2)][zz] == -2) || xx == TreeX - 1) || !IsOkayUpY(x, z, xx + 1, yy, zz)) &&
        (((xx > 0 && trees[x][z].themap.themap[xx - 1][yy + (TreeY / 2)][zz] == -2) || xx == 0) || !IsOkayUpY(x, z, xx - 1, yy, zz));
}
bool IsOkayDownY(int x, int z, int xx, int yy, int zz)
{
    return ((yy < 1 && trees[x][z].themap.themap[xx][yy + 1 + (TreeY / 2)][zz] == -2) || yy == 0) &&
        ((yy > -(TreeY / 2) && trees[x][z].themap.themap[xx][yy - 1 + (TreeY / 2)][zz] == -2) || yy == -(TreeY / 2));
}
bool IsOkayDownX(int x, int z, int xx, int yy, int zz)
{
    return IsOkayDownY(x, z, xx, yy, zz) &&
        (((xx < TreeX - 1 && trees[x][z].themap.themap[xx + 1][yy + (TreeY / 2)][zz] == -2) || xx == TreeX - 1) || !IsOkayDownY(x, z, xx + 1, yy, zz)) &&
        (((xx > 0 && trees[x][z].themap.themap[xx - 1][yy + (TreeY / 2)][zz] == -2) || xx == 0) || !IsOkayDownY(x, z, xx - 1, yy, zz));
}
void Control()
{
    if (window.hasFocus())
    {
        ...
        else
        {
            ...
            if (Keyboard::isKeyPressed(Keyboard::H)) window.setVisible(false);
    ...
    for (int sx = 0; sx < showxx; sx++) for (int sz = 0; sz < showzz; sz++)
    {
        ...
        for (int xx = 0; xx < TreeX; xx++) for (int zz = 0; zz < TreeZ; zz++) for (int yy = -(TreeY / 2); yy <= (TreeY / 2); yy++)// * (god.pos.y > 0.5)
            if (trees[x][z].themap.themap[xx][yy + (TreeY / 2)][zz] != -2)
                ...
                {
                    int need = 0; bool net = true, uwu = true;
                    glMatrixMode(GL_MODELVIEW); glLoadIdentity(); 
                    gluLookAt(god.pos.x, god.pos.y, god.pos.z,
                        god.pos.x + sin(god.horang * 0.01745),
                        god.pos.y + sin(god.verang * 0.01745),
                    ...
                    if (yy < 1)
                    {
                        if (sunmode)
                        {
                            ...
                            float ener = pow(7.2, (1.0 + double(-1 * yy) * 0.06)) / ((double)need1 * 0.012 + 1.0f);
                            glDisableClientState(GL_COLOR_ARRAY);
                            glColor4f(((ener - 1.25) / (16.2)) * 0.8 + 0.2, ((ener - 1.25) / (16.2)) * 0.8 + 0.2, 0.2, 0.5 + 0.5 * mode);
                        }
                        else
                        {
                            for (int yyy = yy + 1; yyy < 1; yyy++) if (trees[x][z].themap.themap[xx][yyy + (TreeY / 2)][zz] != -2 && net) need++; else net = false;
                            glScalef(1.0f, need + 1.0f, 1.0f); yy += need; net = true;
                            if (need == 0)
                            {
                                if (xx > 0 && trees[x][z].themap.themap[xx - 1][yy + (TreeY / 2)][zz] != -2 && IsOkayDownY(x, z, xx - 1, yy, zz)) uwu = false;
                                else
                                {
                                    for (int xxx = xx + 1; xxx < TreeX; xxx++)
                                        if (trees[x][z].themap.themap[xxx][yy + (TreeY / 2)][zz] != -2 && IsOkayDownY(x, z, xxx, yy, zz) && net) need++; else net = false;
                                    glScalef(need + 1.0f, 1.0f, 1.0f); net = true;
                                    if (need == 0)
                                    {
                                        if (zz > 0 && trees[x][z].themap.themap[xx][yy + (TreeY / 2)][zz - 1] != -2 && IsOkayDownX(x, z, xx, yy, zz - 1)) uwu = false;
                                        else
                                        {
                                            for (int zzz = zz + 1; zzz < TreeZ; zzz++)
                                                if (trees[x][z].themap.themap[xx][yy + (TreeY / 2)][zzz] != -2 && IsOkayDownX(x, z, xx, yy, zzz) && net) need++; else net = false;
                                            glScalef(1.0f, 1.0f, need + 1.0f);
                                        }
                                    }
                                }
                            }
                    ...
                    else
                    {
                        ...
                        {
                            for (int yyy = yy + 1; yyy <= (TreeY / 2); yyy++) if (trees[x][z].themap.themap[xx][yyy + (TreeY / 2)][zz] != -2 && net) need++; else net = false;
                            glScalef(1.0f, need + 1.0f, 1.0f); yy += need; net = true;
                            if (need == 0)
                            {
                                if (xx > 0 && trees[x][z].themap.themap[xx - 1][yy + (TreeY / 2)][zz] != -2 && IsOkayUpY(x, z, xx - 1, yy, zz)) uwu = false;
                                else
                                {
                                    for (int xxx = xx + 1; xxx < TreeX; xxx++) 
                                        if (trees[x][z].themap.themap[xxx][yy + (TreeY / 2)][zz] != -2 && IsOkayUpY(x, z, xxx, yy, zz) && net) need++; else net = false;
                                    glScalef(need + 1.0f, 1.0f, 1.0f); net = true;
                                    if (need == 0)
                                    {
                                        if (zz > 0 && trees[x][z].themap.themap[xx][yy + (TreeY / 2)][zz - 1] != -2 && IsOkayUpX(x, z, xx, yy, zz - 1)) uwu = false;
                                        else
                                        {
                                            for (int zzz = zz + 1; zzz < TreeZ; zzz++)
                                                if (trees[x][z].themap.themap[xx][yy + (TreeY / 2)][zzz] != -2 && IsOkayUpX(x, z, xx, yy, zzz) && net) need++; else net = false;
                                            glScalef(1.0f, 1.0f, need + 1.0f);
                                        }
                                    }
                                }
                    ...
                    if (uwu) glDrawElements(GL_TRIANGLES, 72, GL_UNSIGNED_INT, NULL);
                }
    }
}
void Init()
...
int main()
{
    ...
    while (work)
    {
        ...
        if (Keyboard::isKeyPressed(Keyboard::V) && Keyboard::isKeyPressed(Keyboard::B)) 
        { 
            window.setVisible(true); while (!window.hasFocus()); TimesGone = 0;
            cout << "BestGenom[" << GenLength << "][6] = { ";
            for (int x = 0; x < GenLength; x++)
            {
                cout << "{";
                for (int y = 0; y < 6; y++) 
                    if (y != 5) cout << trees[thebest[0]][thebest[1]].Genom[x][y] << ", ";
                    else cout << trees[thebest[0]][thebest[1]].Genom[x][y];
                if (x != GenLength - 1) cout << " }, "; else cout << " } }";
                cout << endl;
            }
        }
        if (window.hasFocus())
